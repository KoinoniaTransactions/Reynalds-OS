...
describe("auth permissions", () => {
  ...
  it("allows client users to view and upload portal documents", () => {
    ...
    expect(
      can(client, "client-portal:documents:upload")
    ).toBe(true);
  });
  ...
  it("allows client users to request showing support without object-admin access", () => {
    ...
    expect(
      can(client, "client-portal:showings:view")
    ).toBe(true);
    expect(
      can(client, "client-portal:showings:create")
    ).toBe(true);
  ...
  it("allows client users to submit access requests without object-admin access", () => {
    ...
    expect(
      can(client, "client-portal:access:view")
    ).toBe(true);
    expect(
      can(client, "client-portal:access:update")
    ).toBe(true);
  ...
  it("allows operations employees to assign work", () => {
    ...
    expect(
      can(employee, "employee-portal:view")
    ).toBe(true);
    expect(
      can(employee, "employee-portal:assignments:update")
    ).toBe(true);
    expect(
      can(employee, "employee-portal:reviews:view")
    ).toBe(true);
  });

  it("keeps client users out of the employee portal", () => {
    expect(
      can(getMockClientUser(), "employee-portal:view")
    ).toBe(false);
  });

  it("limits showing providers to assigned work", () => {
    const permissions =
      rolePermissions["Showing Provider"];
  ...
  it("allows contract support to draft and request approval", () => {
    const permissions =
      rolePermissions["Contract Support"];
  ...
  it("allows clients to approve portal documents without staff document tools", () => {
    ...
    expect(
      can(client, "client-portal:documents:approve")
    ).toBe(true);
    expect(
      can(client, "document-workspace:view")
    ).toBe(false);
    expect(
      can(client, "document-workspace:send")
    ).toBe(false);
  });
  ...
  it("allows clients to view, set up, and pay their own billing", () => {
    ...
    expect(
      can(client, "client-portal:billing:view")
    ).toBe(true);
    expect(
      can(client, "client-portal:billing:setup")
    ).toBe(true);
    expect(
      can(client, "client-portal:billing:pay")
    ).toBe(true);
    expect(
      can(client, "billing-workspace:payments:process")
    ).toBe(false);
  });

  it("allows the owner to use the client invoice payment path", () => {
    expect(
      can(getMockUser(), "client-portal:billing:pay")
    ).toBe(true);
  });
  ...
  it("falls unknown provider roles back to viewer permissions", () => {
    ...
    expect(normalizeRoleName("External Admin")).toBe(
      "Viewer"
    );
    expect(user.role).toBe("Viewer");
    expect(
      getPermissionsForRole("External Admin")
    ).toEqual(rolePermissions.Viewer);
    expect(
      can(user, "client-portal:view")
    ).toBe(false);
  });
