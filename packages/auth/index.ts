export type Permission =
  ...
  | "client-portal:transactions:view"
  | "client-portal:transactions:create"
  | "client-portal:transactions:update"
...
export function getMockUserId(workspaceId: string, role: string): string {
  const normalizedRole = normalizeRoleName(role);

  if (workspaceId === "wks_koinonia" && normalizedRole === "Owner") {
    return "usr_owner";
  }

  return [
    "usr_mock",
    normalizeMockIdSegment(workspaceId),
    normalizeMockIdSegment(normalizedRole)
  ].join("_");
}

function normalizeMockIdSegment(value: string): string {
...
export function getMockUser(): AuthUser {
  const workspaceId = process.env.ROS_MOCK_WORKSPACE_ID ?? "wks_koinonia";

  return createAuthUser({
    id: getMockUserId(workspaceId, "Owner"),
    workspaceId,
...
export function getMockClientUser(): AuthUser {
  ...
  return createAuthUser({
    id: getMockUserId(workspaceId, "Client"),
    workspaceId,
...
export function getMockEmployeeUser(): AuthUser {
  ...
  return createAuthUser({
    id: getMockUserId(workspaceId, "Operations"),
    workspaceId,
