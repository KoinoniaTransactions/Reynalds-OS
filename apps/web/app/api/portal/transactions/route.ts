...
export async function POST(request: Request) {
  try {
    ...
    if (input.intakeRequestId) {
      ...
      const existingTransaction = recentTransactions.find((transaction) => asRecord(transaction.data)?.intakeRequestId === input.intakeRequestId);
      if (existingTransaction) {
        return NextResponse.json({ clientObject: null, relationship: null, reusedClient: false, reusedIntake: true, transaction: existingTransaction });
      }
    }

    const normalizedClientName = input.clientName ? normalizeClientIdentityName(input.clientName) : null;
    ...
    const matchedClient = normalizedClientName
      ? existingRelationships.find((relationship) => {
          ...
          const storedIdentity = typeof data?.normalizedClientName === "string" ? data.normalizedClientName : normalizeClientIdentityName(relationship.name);
    ...
    const result = await prisma.$transaction(async (tx) => {
      const clientObject = input.clientName
        ? matchedClient ?? await tx.rosObject.create({
            data: {
              workspaceId: actor.workspaceId,
              objectType: clientRelationshipObjectType,
              ...
              data: { normalizedClientName, relationshipKind: "client_household", source: "transaction_intake" } as Prisma.InputJsonObject
            }
          })
        : null;

      const hasConfirmedIdentity = Boolean(input.side && input.stage && input.clientName && (input.side === "buyer" || input.propertyAddress));
      ...
      return { clientObject, relationship, reusedClient: Boolean(matchedClient), reusedIntake: false, transaction };
    });
    ...
    if (error instanceof ClientTransactionValidationError) return NextResponse.json({ error: error.message }, { status: 400 });
    if (error instanceof Error && (error.name === "PrismaClientInitializationError" || error.message.includes("Can't reach database server") || error.message.includes("ECONNREFUSED"))) {
      return NextResponse.json({ error: "Transaction intake storage is temporarily unavailable." }, { status: 503 });
    }
    throw error;
  }
}
