...
export async function PATCH(request: Request, { params }: Params) {
  try {
    ...
    const previousValue = {
      ...
      nextAction: billingSetupRequest.nextAction,
      status: billingSetupRequest.status
    };
    const nextData = buildBillingSetupStatusData(billingSetupRequest.data, input, actor);
    const updatedBillingSetupRequest = await prisma.$transaction(async (tx) => {
      const updated = await tx.rosObject.update({
        where: { id: billingSetupRequest.id },
        data: {
          data: nextData,
          health: getBillingSetupHealth(input.status),
          nextAction: buildBillingSetupStatusNextAction(input.status),
          status: input.status
        }
      });
      const newValue = {
      ...
      await tx.timelineEvent.create({
        data: {
          ...
          actorId: actor.id,
          eventType: "billing_setup.status.updated",
          summary: `Billing setup status updated: ${billingSetupRequest.name} is ${input.status}`,
          previousValue,
          newValue
        }
      });
      ...
      await tx.auditEvent.create({
        data: {
          ...
          action: "portal.billing_setup.status.updated",
          ...
          metadata: {
            ...
            previousStatus: billingSetupRequest.status,
            status: input.status
          }
        }
      });
    ...
    return NextResponse.json({ billingSetupRequest: updatedBillingSetupRequest });
  } catch (error) {
    return handleBillingSetupStatusError(error);
  }
}

function buildBillingSetupStatusData(
  currentData: unknown,
  input: ReturnType<typeof validateBillingSetupStatusUpdateInput>,
  actor: AuthUser
  ...
  if (input.paymentMethodSummary) {
    data.paymentMethodSummary = input.paymentMethodSummary;
  }

  if (input.processorReference) {
    data.processorReference = input.processorReference;
  }
...
function handleBillingSetupStatusError(error: unknown) {
