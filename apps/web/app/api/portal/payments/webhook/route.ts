...
import { getStripeInvoiceTransitionDecision } from "../../../../../lib/stripe-invoice-payment-sessions";
...
export async function POST(request: Request) {
  ...
  const webhookSecret =
    process.env.KOINONIA_PAYMENT_WEBHOOK_SECRET;
  ...
  let summary =
    summarizeStripePortalEvent(event);
  const workspaceId =
    summary.workspaceId ||
  ...
  if (
    summary.action ===
      "billing_setup_ready" &&
    summary.billingSetupRequestId
  ) {
    const billingSetupRequestId =
      summary.billingSetupRequestId;

    summary =
      await enrichSetupPaymentMethodSummary(
        summary
      );
    ...
    await markBillingSetupReady({
      ...
      eventId: event.id,
      paymentMethodBrand:
        summary.paymentMethodBrand,
      paymentMethodExpirationMonth:
        summary.paymentMethodExpirationMonth,
      paymentMethodExpirationYear:
        summary.paymentMethodExpirationYear,
      paymentMethodLast4:
    ...
    await recordStripeEvent({
      action:
        "portal.payment.stripe_event.received",
      eventId: event.id,
      eventType: summary.eventType,
      metadata: {
        hasBillingSetupRequestId:
          Boolean(
            summary.billingSetupRequestId
          ),
        hasInvoiceId:
          Boolean(summary.invoiceId)
      },
      summary:
        `Stripe event received: ${summary.eventType}`,
      workspaceId
    });
  }
...
async function markBillingSetupReady(input: {
  ...
  if (!billingSetupRequest) {
    await recordStripeEvent({
      action:
        "portal.payment.stripe_event.unmatched",
      ...
      summary:
        "Stripe billing setup event did not match a portal billing setup request.",
      workspaceId:
        input.workspaceId
    });
  ...
  const nextData =
    ...
    typeof billingSetupRequest.data ===
      "object" &&
    !Array.isArray(
      billingSetupRequest.data
    )
      ? {
          ...(billingSetupRequest.data as Record<
            string,
            unknown
          >)
        }
  ...
  const customerBillingProfileId =
    readOptionalString(
  ...
  nextData.paymentMethodSummary =
    input.paymentMethodSummary ??
    nextData.paymentMethodSummary;
  nextData.processorReference =
  ...
  nextData.processorVerifiedAt =
    verifiedAt;
  nextData.processorWebhookEventId =
    input.eventId;
  ...
  await prisma.$transaction(
    async (tx) => {
      ...
      let customerBillingProfileUpdated =
        false;
            ...
            mergeProcessorPaymentMethodProfileData(
              customerBillingProfile.data,
              {
                ...
                customerReference:
                  input.customerReference,
                expirationMonth:
                  input.paymentMethodExpirationMonth,
                expirationYear:
                  input.paymentMethodExpirationYear,
                last4:
          ...
          await tx.timelineEvent.create({
            data: {
              ...
              objectId:
                customerBillingProfile.id,
              eventType:
                "billing_profile.payment_method.verified",
              summary:
                `Stripe confirmed a payment method for ${customerBillingProfile.name}.`,
              newValue: {
      ...
      await tx.timelineEvent.create({
        data: {
          ...
          eventType:
            "billing_setup.processor.verified",
          summary:
            `Stripe confirmed payment setup for ${billingSetupRequest.name}.`,
          previousValue: {
            status:
      ...
      await tx.auditEvent.create({
        data: {
          workspaceId:
            input.workspaceId,
          action:
            "portal.payment.stripe_event.processed",
          actorEmail: "stripe",
          subjectType: "StripeEvent",
          subjectId: input.eventId,
          summary:
            `Stripe payment setup event processed for ${billingSetupRequest.name}.`,
          metadata: {
            billingSetupRequestId:
              billingSetupRequest.id,
            customerBillingProfileId:
...
async function updateInvoicePaymentStatus(
  ...
  if (!existing) {
    await recordStripeEvent({
      ...
      summary:
        "Stripe payment event did not match a portal invoice.",
      workspaceId:
        input.workspaceId
    });
  ...
  if (transition !== "process") {
    await recordStripeEvent({
      action:
        "portal.payment.stripe_event.ignored",
      eventId: input.eventId,
      eventType: input.status,
      metadata: {
        currentStatus:
      ...
      summary:
        transition === "redundant"
          ? `Stripe payment event ignored because invoice ${existing.id} is already ${input.status}.`
          : `Stripe payment event ignored because it would regress invoice ${existing.id} from ${existing.status} to ${input.status}.`,
      workspaceId:
        input.workspaceId
    });
    ...
    async (tx) => {
      ...
      await tx.timelineEvent.create({
        data: {
          ...
          summary:
            `Stripe updated invoice payment status to ${input.status}.`,
          previousValue: {
            paidAt:
      ...
      await tx.auditEvent.create({
        data: {
          ...
          subjectId: input.eventId,
          summary:
            `Stripe payment event processed for invoice ${invoice.id}.`,
          metadata: {
            hasPaymentMethodSummary:
              Boolean(
                input.paymentMethodSummary
