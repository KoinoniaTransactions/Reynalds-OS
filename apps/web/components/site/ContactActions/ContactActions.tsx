...
export function ContactActions({ variant = "cards" }: ContactActionsProps) {
  ...
  function trackContactAction(key: string, title: string) {
    trackGoogleAnalyticsEvent("contact_action", {
      contact_method: key,
      contact_label: title
    });
  }
      ...
      >
        {actions.map((action) => (
          <a
            ...
            onClick={() => {
              if (!action.placeholder) trackContactAction(action.key, action.title);
            }}
          >
